...
definition(
    ...
    namespace: "schwark",
    author: "Schwark Satyavolu",
    description: "Allows you to hit a cURL command when a button is pressed",
...
def setState(response, browserSession) {
	...
	if(browserSession.state.token) state.token = browserSession.state.token
}

private def getRecipe(command, id=null) {
	...
	def STEPS = [
			...
			'vehicles': [name: 'vehicles', uri: "${domain}/api/1/vehicles", state: [cars: 'json:response'], headers: ['Authorization': "Bearer ${token}"], processor: this.&createSwitches],	
			]
	def RECIPES = [
				'token': ['secret', 'auth']
			]
...
def runCommand(command, id=null, browserSession=[:]) {
	...
	navigateUrl(getRecipe(command, id), browserSession)
}
...
private def navigateUrl(recipe, browserSession=[:]) {
    def params = recipe instanceof java.util.List ? recipe.pop() : recipe
	...
	if(params.uri) {
		...
		params.uri = fillTemplate(params.uri, state + browserSession.vars + browserSession.state)
		...
		params.headers['Accept'] = 'application/json,text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8'
        ...
        params.headers.each {name, value ->
        	params.headers[name] = fillTemplate(value, state + browserSession.vars + browserSession.state)
        }
...
def doDeviceSync(){
	...
	navigateUrl(getRecipe('vehicles'))
}
...
def createSwitches(response=null, browserSession=[:]) {
	log.debug("Creating Tesla Switches with ${browserSession && browserSession.state ? browserSession.state.cars : ''}... ")

	def PREFIX = getPrefix()
	def CARS = browserSession && browserSession.state ? browserSession.state.cars : null
	if(!CARS) return

	// add missing devices
	CARS.each() { map ->
		def id = map['id']
		def name = map['display_name']
		...
		if(map.button && !device) {
			...
			carSwitch.setId(id)
			carSwitch.setVin(map['vin'])
			carSwitch.setVehicleId(map['vehicle_id'])
		}
	}
